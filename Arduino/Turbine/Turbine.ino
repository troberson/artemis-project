...
while(1)
{
  ...
  readRPM();
  readCurrent();
  printCurrent();  //only want to print current once per cycle, but need to read current often
  //readWindSpeed();
  //readAnemometer();
 ...
 if(RV_Wind_ADunits>600&& currentRPM>800)// if wind speed is greater than 8 m/s then shorten delay time and shrink correction amounts
  {
    delayTime = 1000;
  ...
  {
    if(currentRPM<previousRPM-coastThreshold)//check if already slowing down--------maybe adjust coast threshold
    {
      //do nothing
  ...
  {
    if(currentRPM>previousRPM+coastThreshold)//check if already speeding up--------
    {
      //do nothing
    ...
    else//not slowing down
    {
      if(currentRPM<desiredRPM-large_threshold_margin)//too slow need large difference
      {
        ...
        else
        {
           ...
           bigSpeedUp = 0;
           CurrentCheckDelay(delayTime);//delay to allow to speed back up hopefully
...
{
  int durationThreshold = 250;//only affects what to print make sure its between large and small duration values
 ...
 while(micros()/1000-currentTime<=duration)//while we havent reached the duration time
 {
  digitalWrite(pitch_direction_pin, dir);  //set direction
  analogWrite(pitch_enable_pin, spd);  //set speed (with pwm)

  if(digitalRead(limit1)==1||digitalRead(limit2)==1)//check if either limit switch was hit
  {
    break;
  }
 }
...
void stopPitching()
{
  digitalWrite(pitch_enable_pin,0);
}
...
int windSpeedLevel()
{
  ...
  for(int i = 0; i<numChecks; i++)//collect 5 different wind speed samples already averaged
  {
...
int loadSelection()//could change this to a void and just send a message instead of returning the load value
//would need to take RPM into account though first
{
  ...
  int total = 0;
    ...
    for(int i = 0;i<5;i++)//check the first element and see if its the same as the rest
    {
